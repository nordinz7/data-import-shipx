...
export const convertToShipCon = (
  shipConCompanies: ShipperConsignee[],
  shipConAddresses: ShipperConDelivery[],
  opts: Opts
) => {
  const groupedAddresses = groupBy(shipConAddresses, "ShipConCode");
  const groupedAreaCodes = groupBy(
    cloneJsonDataFromSheet(opts.workbook.Sheets["AdmArea"]),
    "AreaCode"
  );

  return shipConCompanies.flatMap((company) => {
    const foundAddresses = groupedAddresses[company.Code] || [];

    const addresses = foundAddresses
      .map((address) => {
        ...
        return {
          ...
          zone:
            groupedAreaCodes[address["AreaCode"]]?.[0]?.AreaZone ||
            DEFAULT_IF_REQUIRED_NOT_FOUND,
