...
function loadLatLongDB(bu: string) {
  ...
  const filePath = path.join(
    __dirname,
    "../latlongdb",
    `sl-api-${bu.replace("yh", "")}.ResLocation.json`
  );
  ...
  switch (sheetName) {
    ...
    case "AdmLocation": {
      const data = getCompanies(
        cloneJsonDataFromSheet(opts.workbook.Sheets[sheetName]),
        opts
      ).filter((row: AdmCustomer) =>
        sheetName === "AdmCustomer"
          ? row["SageMappingCode"] !== "NULL" && row["SageMappingCode"]
          : row
      );
    ...
    case "AdmShipperConsignee": {
      ...
      const shipConDelSheet = opts.workbook.SheetNames.find((sheetName) =>
        targetSheets.includes(sheetName)
      );

      if (!shipConDelSheet) {
        console.log(
          `Sheet "ShipperConDelivery" not found in ${opts.fileBaseName}`
        );
        return;
      }

      const shipConCompanies = cloneJsonDataFromSheet(
        opts.workbook.Sheets[sheetName]
      );
      const shipConAddresses = cloneJsonDataFromSheet(
        opts.workbook.Sheets[shipConDelSheet]
      );
...
function extractFromWorkbook(workbook: XLSX.WorkBook, filePath: string) {
  ...
  const generateOutput = (fileSuffixName: string, content) => {
    const csvFilePath = path.join(
      outputDir,
      `${fileBaseName}_${fileSuffixName}.csv`
    );
    fs.writeFileSync(csvFilePath, content);
  };

  sheetNames.forEach((sheetName) =>
    extractFromSheet(sheetName, {
      workbook,
      generateOutput,
      fileBaseName,
      buCode: getBuCode(filePath),
    })
  );
}
